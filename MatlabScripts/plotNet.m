...
for i=1:layers(1)
        ...
        edges.n1(i)      = i;
        edges.n2(i)      = layers(1)+jNeuron;
        edges.weights(i) = 1;
        edges.names{i}   = ['$w_{',num2str(jNeuron-1),num2str(i-1),'}$'];
end
...
text(3,max(nodes.y),str,'interpreter','latex','fontSize',28,HorizontalAlignment = 'center')
...
function PlotNet(nodes,edges,t)
    ...
    for i=1:length(edges.n1)
        ...
        text(x,y,edges.names(i),'interpreter','latex',FontSize=28, ...
